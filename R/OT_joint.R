...
OT_joint=function(inst, maxrelax=0.0, lambda_reg:=0.0, percent_closest=0.2, norme=0, aggregate_tol=0.5, full_disp=false, solver_disp=false){
    ...
    Xvalues = as.matrix(Xobserv);
    norme = 1;
    if (norme == 0){
        dist_X = pairwise(Hamming(), transpose(Xvalues),transpose(Xvalues), dims=2)}
    else if (norme == 1){
        dist_X = pairwise(Cityblock(), transpose(Xvalues),transpose(Xvalues), dims=2)
    }
    voisins_X = as.numeric(dist_X <= 1);
    ...
    estim_XA = numeric(0)
    for (x in (1:nbX)){
    estim_XA = c(estim_XA,Dict([(x, length(indXA[x])/nA))}
    estim_XB = numeric(0)
    for (x in (1:nbX)){
    estim_XB = c(estim_XB,Dict([(x, length(indXB[x])/nB))}
    estim_XA_YA = numeric(0)
    for (x in (1:nbX)){
      for (y in Y){
    estim_XA_YA = c(estim_XA_YA,Dict([((x,y), length(indXA[x][(Yobserv[indXA[x]] == y)])/nA) ])}
      estim_XB_ZB= numerc(0)
    for (x in (1:nbX)){
      for (z in Z){
    estim_XB_ZB = c(estim_XB_ZB,Dict([((x,z), length(indXB[x](Zobserv[indXB[x]nA] == z)])/nB) ]))}}
   ...
   return(Solution(time()-tstart, [sum(gammaA_val[x,y,z] for x in 1:nbX) for y in Y, z in Z], [sum(gammaB_val[x,y,z] for x in 1:nbX) for y in Y, z in Z], estimatorZA, estimatorYB));
}
