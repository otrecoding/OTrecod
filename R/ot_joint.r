...
#' application of optimal transportation which aims is to search for a bijective mapping between the joint distributions of \code{(Y,X)} and \code{(Z,X)} in A and B (see (2) for more details).
...
#' In the same way, for a given profile of covariates \code{Pj}, an individual i will be considered as a neighbor of \code{Pj} if \eqn{dist(i,P_j) < prox.dist \times max(dist(i,P_j))} where \eqn{prox.dist} will be fixed by user.
#'
