...
proxim_dist <- function(data_file, indx_DB_Y_Z = 1:3, norm = "E", prox = 0.30) {
  ...
  indY <- indZ <- list()
  for (m in Y) {
    indY[[m]] <- which(Yobserv[1:nA] == m)
  }
  for (m in Z) {
    indZ[[m]] <- which(Zobserv[(nA + 1):(nA + nB)] == m)
  }
  ...
  if (nrow(D) != nrow(na.omit(D))) {
    ...
    tab_pb <- coord[is.na(apply(coord, 1, function(x) {
      D[x[1], x[2]]
    })), ]
    ...
    message("!!! Because of the presence of NAs in distance matrix, the process stopped. ",
      ...
      "To help you, the indexes of rows are listed in the returned object.",
      sep = "\n"
    )
  ...
  for (i in (1:n_Xval)) {
    ...
    indXA[[i]] <- which(distA < prox * max(distA, na.rm = TRUE))
    names(indXA[[i]]) <- NULL
    indXB[[i]] <- which(distB < prox * max(distB, na.rm = TRUE))
    names(indXB[[i]]) <- NULL
  ...
  return(list(
    FILE_NAME = file_name, nA = nA, nB = nB, Xobserv = Xobserv, profile = unique(Xobserv),
