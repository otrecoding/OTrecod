...
#' training_error()
#' 
#' A function which gives an idea of the accuracy on training set of the OT algorithm
#'
...
training_accuracy = function(listdata,rate,seed1 = sample(1:999999,1)){
  ...
  xx_new         = cost(prep_new,"G")
  listdata_new   = affectation(xx_new$solution,prep_new,dist_choice = "G",which.DB = "BOTH")
  ...
  predY2    = round(sum(diag(table(Ytheo[[1]],yt1)))*100 / sum(table(Ytheo[[1]],yt1)),2)
  predY1    = round(sum(diag(table(Ytheo[[2]],yt2)))*100 / sum(table(Ytheo[[2]],yt2)),2)
