...
We used the dataset called "samp.A" from the StatMatch package (see `?samp.A` for more details). This data set provides a limited number of variables observed at persons levels among those usually collected in the European Union Statistics on Income and Living Conditions Survey (EU–SILC). 

We introduced a recoding problem in this dataset to illustrate our method. 
...
We first created 'c.neti` and `c.neti.bis`  coded voluntarily in 2 distinct encodings:
- Y: first encoding into 6 categories (the original categories)
- Z: second encoding into 4 categories (1:class 1 and 2, 2: class 3 and 4, 3: class 5 and 6)

 Then we separate into two databases`where we observ only  'c.neti` in database 1 and `c.neti.bis` in database 2.
- Participants randomly divided in two databases: `data1` and `data2`
- Unbalanced databases on covariates `data3` and `data4`
We introduce missing data (describe how MAR, MNAR or MCAR). 
...
On database A, by grouping the modalities of the c.neti variable (7 levels) like 1 (best grouping) in 4 modalities (number of modalities of c.neti.bis) the dissimilarity rate between the 2 variables is less than 8%. 
