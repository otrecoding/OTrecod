test_that("verif_OT works", {
  ...
  test1 = OT_outcome(simu_data, quanti = c(3,8), nominal = c(1,4:5,7), ordinal = c(2,6),
  ...
  expect_equal(length(ver1),8)

  expect_equal(is.numeric(ver1[[1]]),TRUE)
  expect_equal(is.numeric(ver1[[2]]),TRUE)

  expect_equal(nrow(ver1[[3]]),length(levels(simu_data[,2]))+1)
  expect_equal(ncol(ver1[[3]]),length(levels(as.factor(simu_data[,3])))+1)

  expect_equal(is.matrix(ver1[[4]]),TRUE)
  expect_equal(dim(ver1[[4]]),c(3,3))

  expect_equal(is.null(ver1[[5]]),TRUE)
  expect_equal(dim(ver1[[6]]),c(1,2))
  expect_equal(is.null(ver1[[7]]),TRUE)
  expect_equal(is.null(ver1[[8]]),TRUE)


  # two overlayed databases, all options
  ver2 = verif_OT(test1, group.clss = TRUE, stab.prob = TRUE)

  expect_equal(ncol(ver2[[5]]),5)
  expect_equal(!is.null(ver2[[7]]),TRUE)
  expect_equal(dim(ver2[[8]]),c(3,5))

  # One database (A), all options
  test2 = OT_outcome(simu_data, quanti = c(3,8), nominal = c(1,4:5,7), ordinal = c(2,6),
                     dist.choice = "G",percent.knn = 0.90, maxrelax = 0,
                     convert.num = 8, convert.clss = 3,
  ...
  expect_equal(length(ver3[[4]]),3)
  expect_equal(dim(ver3[[6]]),c(1,2))
  expect_equal(is.na(ver3[[6]][,1]),TRUE)
  expect_equal(dim(ver3[[8]]),c(1,5))
