...
test_that("mice works",{
  ...
  ind1 = 4:8
  imp1 = imput_cov(simu_data, indcol = ind1, R_mice = 3, meth = c("logreg","polyreg","polr","logreg","pmm"))
  ...
  # output 1
  expect_equal(dim(imp1[[1]]),dim(simu_data))
  ...
  expect_equal(nrow(imp1[[3]]),nrow(simu_data))
  expect_identical(colnames(imp1[[3]]),colnames(simu_data)[ind1])
  # expect_identical(lapply(imp1[[3]],typeof),lapply(simu_data[ind1],typeof))

  # output 4
  expect_equal(length(imp1[[4]]),3)
  expect_identical(colnames(imp1[[4]][[1]]),colnames(simu_data)[ind1])


  ind3 = c(4,6:8)
  imp3 = imput_cov(simu_data, indcol = ind3, R_mice = 3, meth = c("logreg","polr","logreg","pmm"))

  expect_identical(colnames(imp3[[3]]),colnames(simu_data)[ind3])

  ind4 = c(6:8,4)
  imp4 = imput_cov(simu_data, indcol = ind4, R_mice = 3, meth = c("polr","logreg","pmm","logreg"))

  expect_identical(colnames(imp4[[3]]),colnames(simu_data)[ind4])
